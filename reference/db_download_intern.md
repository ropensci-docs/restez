...
Download .seq.tar files from the latest GenBank release. The user
interactively selects the parts of GenBank to download (e.g. primates,
plants, bacteria ...). This is an internal function so the download can
be wrapped in `while()` to enable persistent downloading.
